...
void MathLeafEdit::maybeFunction(QString text) {updateContStr();
    if(text == ":") {
        ...
        insertText("="); m_isFunction = true;
        MathLeafEdit* newL = new MathLeafEdit(resultString(), m_data, m_parentPageMathItem, this);
        insertMathLeaf(newL);
        newL->updateBoundingRectAndLayout();
    } else if (text == "*") {
        m_paddingH = 0;
        insertText(QChar(0x00B7));
        m_isFunction = true;
    } else if (text == "/") {
...
//qDebug() << "    QrectF = " << numer->m_boundingRectangle << "/" << denom->m_boundingRectangle;
    if (numer && denom) {
        ...
        qreal maxW = qMax(numW, denW);
        painter->drawLine(0, numH+1.75*m_paddingV, maxW, numH+1.75*m_paddingV);
    }
    painter->restore();
}
...
void MathLeafEdit::setCursorBeforeChild(MathLeafEdit *child) {
    if (!child) {
        qWarning() << "MathLeafEdit::setCursorBeforeChild: Attempted to remove a set cursor before a NULL child.";
    }
...
void MathLeafEdit::keyPressEvent(QKeyEvent* event) {updateContStr();
    ...
    if (charToInsert.length() == 1){
        ...
        if(!m_isFunction) {
            if ((qc >= 'A' && qc <= 'Z') || (qc >= 'a' && qc <= 'z') || (qc >= '0' && qc <= '9') ||
                (qc == '\\') || (qc == '.') || (qc == ',') || (qc == '_')) {
            ...
            updateContStr();
            if (qc == '*') qc = QChar(0x00B7); // replace * with dot
            if ((qc == '+' || qc == '-' || qc == QChar(0x00B7)) &&
                (m_content.at(0) == "+" || m_content.at(0) == "-" || m_content.at(0) == QString(QChar(0x00B7)) ||
                 m_content.at(0) == ":=" || m_content.at(0) == "=")) {
                m_content[0] = QString(qc);
                if (qc == '+' || qc == '-') m_paddingH = 3; else m_paddingH = 0;
                //updateBoundingRectAndLayout();
            } else if ((qc == ':') && (m_content.at(0) == "+" || m_content.at(0) == "-" || m_content.at(0) == QString(QChar(0x00B7)))) {
                m_content[0] = QString(":=");
                //updateBoundingRectAndLayout();
            } else {
                //insertText(event);
    ...
    if (event->key() == Qt::Key_Backspace) {
        ...
        //emit functionEntered(m_content.at(m_cursorPos).toString());
    } else {
...
void MathLeafEdit::updateBoundingRectAndLayout() {updateContStr();
...
void MathLeafEdit::updateChildPositions() {
    ...
    if (m_contStr==QString("<OBJ>/<OBJ>")) { // This is a division (fraction)
        ...
        if (numer && denom) {
            ...
            qreal numY = 0.0;
            qreal denY = numH + 2.5*m_paddingV;
            numer->setPos(numX, numY); denom->setPos(denX, denY);
            m_centerHeight = numH + 1.25+m_paddingV;
        ...
        for (int i=0; i<m_content.size(); i++) {
            if (isMathLeafEditAtContent(i)) {
                subscriptActive = false;
                MathLeafEdit* child = getMathLeafEditAtContent(i);
                qreal childY = m_centerHeight - child->centerHeight() - m_paddingV;
...
QList<qreal> MathLeafEdit::getValue() {
    return m_value;
}
...
void MathLeafEdit::compute() {
    ...
    if (isRoot) { qDebug()<< "MathLeafEdit::compute: This is the root MathLeafEditObject.";
        ...
        if (isString(tokenList[0]) && isMathLeafEdit(tokenList[1])) {
            m_name = getString(tokenList[0]);
            bool canConvertToDouble; qreal myReal = m_name.toDouble(&canConvertToDouble);
            if (canConvertToDouble) {
                ...
                MathLeafEdit* func = getMathLeafEdit(tokenList[1]);
                if (func->isAssignmentFunction()) {
                    if (m_data->contains(m_name)) {
                        m_mathWarning = tr("WARNING: Redefinition of existing variable '%1'.").arg(m_name);
                    }
                    func->compute();
                    m_value = func->getValue();
qDebug() << tr("m_name = %1, m_value = %2").arg(m_name).arg(m_value[0]);
                    m_data->setValue(m_name, m_value);
                } else if (func->isResultFunction()) {
                    if (m_data->contains(m_name)) {
                        m_value = m_data->getValue(m_name);
                        func->setValue(m_value);
                        QString resStr = func->resultString();
                        func->getMathLeafEditAtContent(1)->m_content[0] = resStr;
                    } else {
        ...
        if (isExpression()) { // A MathLeafEdit object containing something like -2*a+b+3*c
            ...
            QList<QList<qreal>> operandList;
            for (int i=0; i<tokenList.size(); i++) {
                if (isString(tokenList[i])) {
                    QString S = getString(tokenList[i]);
                    if (isDoubleString(S)) operandList.append(QList<qreal>{S.toDouble()});
                    else {
                        QList<qreal> value = m_data->getValue(S);
                        if (isValidValue(value)) operandList.append(value);
                        else {
                            m_mathError = tr("ERROR: No valid entry found for '%1'. Not defined yet?").arg(getString(tokenList[i]));
                ...
                } else if (isMathLeafEdit(tokenList[i])) {
                    MathLeafEdit* M = getMathLeafEdit(tokenList[i]);
                    if (M->isBasicOperator()){
                        operatorList.append(M->getStringAtContent(0));
                        operandList.append(QList<qreal>()); // if we found an operator, an empty item goes into the operandList
            ...
            while (operandList.size() > 1) { // we simplyfy the expression, '*' operations before '+' and '-' operations
                QList<int> multOpListIDs; // list with all indices where we have a '*' operator
                for(int i=1; i<operatorList.size(); i++) if (operatorList[i] == QString(QChar(0x00B7))) multOpListIDs.append(i);
                for(auto it = multOpListIDs.crbegin(); it != multOpListIDs.crend(); ++it) { // make all '*' computations first.
                    int ID = *it;
                    operandList[ID-1] = mul(operandList[ID-1], operandList[ID+1]);
                    operatorList.removeAt(ID+1); operatorList.removeAt(ID);
                    operandList.removeAt(ID+1); operandList.removeAt(ID);
                }
                if (startsWithOperator) { // integrate a sign into the first operand
                    if (operatorList[0] == "-") operandList[1] = mul(QList<qreal>{-1.0}, operandList[1]);
                    operatorList.removeAt(0); operandList.removeAt(0); startsWithOperator = false;
                }
                if (operandList.size()<2) break; // expression is already down to one opearand --> exit the while loop
                
                if (operatorList[1] == "+" && operandList.size() >= 3 && operatorList.size() >= 3) { // make a '+' computation
                    operandList[0] = add(operandList[0], operandList[2]);
                    operatorList.removeAt(2); operatorList.removeAt(1);
                    operandList.removeAt(2); operandList.removeAt(1);
                } else if (operatorList[1] == "-" && operandList.size() >= 3 && operatorList.size() >= 3) { // make a '-' computation
                    operandList[0] = sub(operandList[0], operandList[2]);
                    operatorList.removeAt(2); operatorList.removeAt(1);
                    operandList.removeAt(2); operandList.removeAt(1);
            ...
            m_value = operandList[0];
qDebug() << tr("m_value = %1").arg(m_value[0]);
...
qDebug() << tr("sin: m_value = %1").arg(arg1->getValue()[0]);
...
qDebug() << tr("base = %1, exponent = %2").arg(arg1->getValue()[0]).arg(arg2->getValue()[0]);
...
bool MathLeafEdit::isValidValue(QList<qreal> L) {
    if (L.size() == 0) return false;
    if (L.size() == 1) {
        if (L[0] == std::numeric_limits<qreal>::max()) return false;
    }
    return true;
}
bool MathLeafEdit::isDoubleString(const QString &s) {
    bool canConvertToDouble; qreal myReal = s.toDouble(&canConvertToDouble);
...
QString MathLeafEdit::resultString() {
    if (m_content.size()>0) {
        if (isStringAtContent(0)) {
            if (getStringAtContent(0) == "=") {
                ...
                if (parentMathLeafEdit) {
                    if (parentMathLeafEdit->isStringAtContent(0)) {
                        ...
                        QList<qreal> result = m_data->getValue(variableName);
                        QString valStr = QString();
                        if (result.size() == 1) {
                            valStr = tr("%1").arg(result[0]);
                        } else if (result.size() > 1 && result.size() <= 4) {
                            valStr = tr("[%1").arg(result[0]);
                            for (int i=1; i<result.size(); i++) {
                                valStr += tr(", %1").arg(result[i]);
                            }
                            valStr += tr("]");
                        } else if (result.size() > 4) {
                            valStr = tr("[%1, ... ,%2]").arg(result[0], result.last());
                        }
                        return valStr;
                    }
                }
