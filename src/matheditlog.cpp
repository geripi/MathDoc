...
MathEditLog::MathEditLog(Data *d, PageMathItem *parentPageMathItem, QGraphicsObject* parent)
...
MathEditLog::MathEditLog(QString text, Data *d, PageMathItem *parentPageMathItem, QGraphicsObject* parent)
:MathEditLog(d, parentPageMathItem, parent) {
...
void MathEditLog::setFocus(Qt::FocusReason focusReason) {
    ...
    if (cashedChild == m_argument && cashedChild->getRightArrowPressed()) { // leave this towards the right
        ...
        m_base->setFocus();
        m_base->setCursorToEnd();
    } else if (cashedChild == m_base && cashedChild->getRightArrowPressed()) {
        m_argument->setFocus();
        m_argument->setCursorToBegin();
    } else if (cashedChild == m_base && cashedChild->getLeftArrowPressed()) { // leave this towards the left
        setCursorToBegin();
        leftArrow();
    } else if (!cashedChild) {
...
void MathEditLog::setFocusToChild1(Qt::FocusReason focusReason) {
    m_argument->setFocus(focusReason);
}
void MathEditLog::setFocusToChild2(Qt::FocusReason focusReason) {
    m_base->setFocus(focusReason);
}

void MathEditLog::init() {
    updateBoundingRect();
}

void MathEditLog::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
    ...
    qreal rightParX = leftParX +4 + m_argument->getBoundingRectangle().x() + m_argument->getBoundingRectangle().width();
...
void MathEditLog::updateBoundingRect() {
    ...
    QRectF bRectBase = m_base->getBoundingRectangle();
    
    qreal argY = 0.0;
    
    qreal argLength = bRectArg.width() + 2.0*getPaddingH();
    qreal argX = textWidth(normStr) + m_base->getBoundingRectangle().width() + 2.0*getPaddingH();
    
    m_argument->setPos(argX, argY);
    
    qreal baseY = 0.3 * fm.height();
    qreal baseX = textWidth(normStr) - 1.0*getPaddingH();
    
    m_base->setPos(baseX, baseY);
    
    qreal top = qMin(bRectArg.top(), bRectBase.top() + baseY);
    qreal bottom = qMax(bRectArg.bottom(), bRectBase.bottom() + baseY);
...
void MathEditLog::compute(const MathVariable& boolMask) {
    m_argument->compute(boolMask);
    m_base->compute(boolMask);
    setValue(log(m_argument->getValue(), m_base->getValue(), boolMask));
}

void MathEditLog::keyPressEvent(QKeyEvent* event)
{
    //printContent();
...
void MathEditLog::mousePressEvent(QGraphicsSceneMouseEvent* event) {
    event->accept();
    m_base->setFocus();
    QGraphicsObject::mousePressEvent(event);
}

void MathEditLog::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
    event->accept();
    QGraphicsObject::mouseMoveEvent(event);
}

bool MathEditLog::isPartOfFuncStr(QString testStr) {
    bool b = false;
...
bool MathEditLog::hasDescendantFocus() const {
    if (hasFocus()) { return true; }
    if (m_argument->hasDescendantFocus()) { return true; }
    if (m_base->hasDescendantFocus()) { return true; }
    
    return false;
}

QJsonObject MathEditLog::toJson() const {
    QJsonObject object;
    object["type"] = type();
...
void MathEditLog::fromJson(const QJsonObject& object) {
    m_content.clear();
    int type = object["type"].toInt();
    if (type == MathEdit::MELog) {
        ...
        const QJsonObject jsonBase = object["base"].toObject();
        
        MathEditFactory::createChildTree(jsonArgument, getData(), getParentPageMathItem(), m_argument);
        MathEditFactory::createChildTree(jsonBase, getData(), getParentPageMathItem(), m_base);
...
void MathEditLog::initializeParenthesis() {
...
void MathEditLog::updateParenthesisPath(const QPointF& topLeft, qreal totalHeight, bool isLeftParenthesis) {
